...
\begin{zremark}
  定理13.8 其中的条件，对每个$n$，$\lim\limits_{x \to a} f_n(x) = a_n$是必须的。

  即:
  ${f_n(x)}$函数列，在$(a,b)$上一致收敛于$f(x)$。
  对任意$N$,$n > N$，$\lim\limits_{x \to a} f_n(x)$的极限都有可能不存在。
\end{zremark}
\textbf{证明:}

举个反例:
\begin{align*}
  f_n(x) = sin \frac{1}{x - a}, \, \, x \in (a, b)
\end{align*}
这个函数列（其实和$n$是没啥关系的）明显收敛于$f(x) = sin \frac{1}{x - a}$，
对任意$n$，$\lim\limits_{x \to a} f_n(x)$极限不存在。
...
另外，这个推论目的是想说明I只需是内闭一致收敛，定理13.11的结论依然成立，
故只会改动一个条件，否则不满足控制变量这一实验原则；
